...
import eu.recred.fidouaf.RPserver.msg.*;
import eu.recred.fidouaf.facets.Facets;
import eu.recred.fidouaf.facets.TrustedFacets;
import eu.recred.fidouaf.res.util.*;
import eu.recred.fidouaf.stats.Dash;
import eu.recred.fidouaf.storage.AuthenticatorRecord;
import eu.recred.fidouaf.storage.DuplicateKeyException;
...
public class FidoUafResource {
	...
	public Facets facets() {
		...
		String[] trustedIds = ProcessAuxRequests.getTrustedIds();
	...
	@GET
	@Path("/public/uaf/registerFacet/{facetId}")
	public void registerFacet(@PathParam("facetId") String facetId) {
		ProcessAuxRequests.addFacetId(facetId);
	}
...
//		return "https://www.head2toes.org/fidouaf/v1/public/uaf/facets";
	...
	@GET
	@Path("/public/lastAuth/{username}")
	@Produces(MediaType.APPLICATION_JSON)
	public GetLastAuth getLastAuthForUsername(@PathParam("username") String username) {
		GetLastAuth dto = new GetLastAuth();
		dto.username_id = username;
		dto.last_auth_timestamp = null;
		try {
			AuthenticatorRecord authRecord = StorageImpl.getInstance().getLastAuth(username);
			dto.last_auth_timestamp = authRecord.timestamp;
			System.out.println("getLastAuthForUsername: got last auth: " + authRecord.timestamp.toString());
		} catch (Exception ex) {
			System.out.println("getLastAuthForUsername exception: " + ex.getMessage());
			ex.printStackTrace();
		}
		return dto;
	}
	...
	@GET
	@Path("/public/isAuthenticated/{authenticationId}")
	@Produces(MediaType.APPLICATION_JSON)
	public String getAuthenticationId(@PathParam("authenticationId") String authenticationId) {
		Gson gson = new Gson();
		return gson.toJson(new ProcessAuxRequests().getAuthenticated(authenticationId));
	}
}
